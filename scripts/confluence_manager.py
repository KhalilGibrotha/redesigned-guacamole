...
class ConfluenceManager:
    ...
    def create_or_update_page(self, title, content, parent_id=None, labels=None):
        """Create a new page or update existing one with change detection"""
        ...
        if existing_page:
            return self._update_page(existing_page, content, content_hash, labels)
        else:
            return self._create_page(title, content, parent_id, content_hash, labels)
    ...
    def _update_page(self, existing_page, content, content_hash, labels):
        ...
        result = response.json()
        logger.info(f"🔄 Updated page: {existing_page['title']} (ID: {page_id}) v{current_version} → v{result['version']['number']}")
    ...
    def validate_page_content(self, page_id, expected_hash=None):
        ...
        url = f"{self.base_url}/rest/api/content/{page_id}"
        params = {
            'expand': 'body.storage,version,metadata.properties'
        }
    ...
    def publish_documentation_hierarchy(self, docs_structure):
        """Publish a complete documentation hierarchy with validation"""
        results = []
    ...
    def _publish_single_page(self, title, content_file, parent_id=None, labels=None):
        """Publish a single page from a markdown file"""
        ...
        result = self.create_or_update_page(title, content, parent_id, labels)
...
def main():
    ...
    parser.add_argument('--action', choices=['publish', 'validate', 'find'], 
                       default='publish', help='Action to perform')
    ...
    if args.action == 'find' and args.title:
        ...
        if page:
            print(json.dumps({
                ...
                "storage_view_url": f"{manager.base_url}/wiki/plugins/viewstorage/viewpagestorage.action?pageId={page['id']}"
            }, indent=2))
        else:
        ...
        if args.docs_structure and Path(args.docs_structure).exists():
            ...
            # Default discovery
            import subprocess
            result = subprocess.run(['python3', 'scripts/discover_docs_enhanced.py'], 
