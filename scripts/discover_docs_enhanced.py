                ...
                for child_item in os.listdir(item_path):
                    ...
                    if os.path.isfile(child_path) and child_item.endswith('.j2'):
                        ...
                        for nested_child in os.listdir(child_path):
                            ...
                            if (os.path.isfile(nested_child_path) and 
                                nested_child.endswith('.j2') and 
                                nested_child not in [f"{nested_name}.j2", "macros.j2"]):
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', nargs='?', default='structure', 
    ...
    if command == "structure":
        ...
        if args.format == 'json':
            ...
            for folder, info in structure.items():
                print(f"  {folder}: {len(info.get('children', []))} children")
        
    elif command == "children" and len(args.args) > 0:
    ...
    elif command == "nested" and len(args.args) > 1:
        # Output nested section info
        main_folder = args.args[0]
        nested_folder = args.args[1]
